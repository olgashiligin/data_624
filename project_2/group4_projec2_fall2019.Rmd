...
```{r message = F}
hist(ph_data[,'PH'], main = 'pH Value Distribution', xlab = 'pH')
```
...
ph_data %>% 
  gather(-PH, -Brand.Code, key="Var", value="Value") %>% 
    ggplot(aes(x = PH, y=Value, color=Brand.Code)) +
   geom_bar( stat = "identity", show.legend = FALSE) +
  facet_wrap(~ Brand.Code, ncol = 1, scales = "free_y")+
    labs(title = "pH Value Distribution by Brand Code",
       y = "Number of Observations")
```


pH is close to normally distributed variable with the range from 8 to 9. Brand D has lower average pH values compare to other brands.
...
There is no clear linear relationship between predictors and PH.
...
Predictors that are highly correlated between each other AND have low correlation with the target variable - pH will be removed form the list of predictors. This list of variables are presented below:
...
ph_data <- ph_data %>%  select (-c("Balling","Filler.Speed", "Hyd.Pressure1","Carb.Temp", "Carb.Pressure"))
```
...
Tree Based Modles: rpart2, treebag, rf, gbm, cubist

```{r, message=FALSE, results='hide'}
library(caretEnsemble)
set.seed(143)
#  setting up trainControl to 10 folds cross validation
control = trainControl("cv", number = 10, savePredictions=FALSE,  index = createFolds(train.data$PH, 5), verboseIter = FALSE)
# creating list of models to be tested
algorithmList = c("lm", "earth", "svmRadial", "rpart2", "rf","treebag", "gbm", "cubist", "xgbTree")
...
Random Forest and Cubist are best performed models, their average RMSE are the lowest with low variation among the samples.

Let's further tune these models and assess their performance on the test set and after that we will select the final best model.

Tuning Random Forest model and assessing its performance on the test set.

```{r, message=FALSE}
#  tuning rf model
set.seed(143)
rf.grid <- expand.grid(.mtry = (1:30)) 
fit.rf.tuned <- train(PH~., data=train.data, method="rf", trControl=control, importance=T, tuneGrid = rf.grid)
...
Tuning Cubist model and assessing its performance on the test set.

```{r, message=FALSE}
#  tuning cubist model
set.seed(143)
cubistGrid <- expand.grid(committees = c(1, 10, 50, 100), neighbors = c(0, 1, 5, 9))
fit.cubist.tuned <- train( PH~., data = train.data, method = "cubist", verbose = FALSE, tuneGrid = cubistGrid, trControl = control)
# selecting best tune parameters
fit.cubist.tuned$bestTune
plot(fit.cubist.tuned)

# assessing performance of the tuned cubist model on the test set
predictions_cubist_tuned <- fit.cubist.tuned %>% predict(test.data)
cubist_tuned_RMSE = caret::RMSE(predictions_cubist_tuned, test.data$PH)
cubist_tuned_RMSE
```
...
```{r, results='show'}
m1<-cbind(RMSE=rf_tuned_RMSE)
m2<-cbind(RMSE=cubist_tuned_RMSE)
summary = rbind(m1, m2)
rownames(summary) <- c("RF Tuned", "Cubist Tuned")
summary
```

Cubist model also better performed than Random Forest model on the test data. We select Cubist as the best model and use it in making predictions of the pH value.

Predicted vs Actual results of the tuned cubist model on the test set are shown on the scatter plot below.

```{r, message=FALSE}
preds_df <- data.frame(predicted = predictions_cubist_tuned, actual = test.data$PH)
ggplot(preds_df) + geom_point(aes(x = predicted, y = actual)) + geom_smooth(aes(x = predicted, y = actual))
```

Using varImp() calculating variable importance for pH using Cubist model.

```{r, message=FALSE}
varImp(fit.cubist.tuned, scale=FALSE)
plot(varImp(fit.cubist.tuned, scale=FALSE))
```
...
Removing highly correlated predictors and near zero variance predictors.

```{r message = F}
pred_data <- pred_data %>%  select (-c("Balling","Filler.Speed", "Hyd.Pressure1","Carb.Temp","Carb.Pressure"))
```
...
Making predictions of pH and saving it in Team4_Predictions.csv (column name of the predicted pH - "PH_pred")

```{r message = F}
# remove ph to get only predictors
pred_data <- pred_data %>% select(-PH)
# making predictions
new_resp <- predict(fit.cubist.tuned, pred_data)
